...
import com.jlju.docmanager.dao.PrizesMapper;
...
public class PrizeServiceImpl implements PrizeService {
    ...
    public int insert(Prizes p, String filesStr) {
    ...
    public Prizes selectById(String uuid) {
        return pMapper.selectByPrimaryKey(uuid);
    }

    private int update(Prizes prizes) {

        return pMapper.updateByPrimaryKeySelective(prizes);
    }

    @Transactional(readOnly = false)
    public int update(Prizes prizes, String files) {
        int result = update(prizes);

        if (files != null && !"".equals(files)) {
            //删除旧文档
            deleteFilesAndCache(prizes.getPriUuid());
            //上传新文档
            uploadNewFiles(files, prizes);
        }

        return result;
    }

    private void uploadNewFiles(String filesStr, Prizes p) {
